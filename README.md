...
Going forward, I want to improve and scale the system in a few key ways:
* **Automated Startup:** Create a systemd service to run the IDS automatically on boot (without needing manual `sudo` execution).
* **Expanded Detection:** Monitor UDP packets, observe full TCP handshakes, and eventually analyze HTTP payloads.
* **Log Security:** Implement log rotation and restrict access permissions to the log file for better security.
* **Cloud Integration & Scaling:** Scale the project by deploying multiple Pi sensors, centralizing logs to a single dashboard, and integrating with AWS CloudWatch and an Elastic Search SIEM (after resolving my malformed IAM signature exceptions) to give full visibility and alerting from anywhere.